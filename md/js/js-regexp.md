...
正则表达式是一门工具，目的就是为了字符串模式匹配，从而实现搜索和替换功能。详情可以通过[regexper.com](http://regexper.com)这个工具来更加直观的可视化自己的正则表达式。
...
凡是以`(?=regex)`包含的子表达式在匹配过程中都会用来限制前面的表达式的匹配。假设这里有`happy``happily`这里两个单词。如果要匹配`happ`开头的副词，可以用`happ(?=ily)`来匹配。如果是要过滤掉`happ`的开头的副词，可以用<b>负前向查找</b>的正则`happ(?!ily)`，就可以匹配到`happy`单词`happ`的前缀。

#### 后向查找

通过指定一个子表达式，然后从符合这个子表达式的位置出发开始查找符合规则的字符串。比如:`good`和`gooood`都是包含`od`这个后缀，我们可以通过限制`go`这个前缀，就能唯一确定`od`这个单词。

```
/(?<=go)od/
```

其中`?<=regex`的语法就是后向查找。`regex`会作为限制项进行匹配，匹配到这个子表达式后，就会继续向后查找。另外一个限制匹配用`(?<!regex)`这个，叫做负后向查找。在这里与正向查找不同的是，被指定的表达式不能被匹配到／所以如果要查找`good`的`od`也可以写成`/(?<!gooo)od/`。  
...
* 在正则里面都是与，所以不说。
* 非分为两种: <b>字符匹配</b>和<b>子表达式匹配</b>。字符匹配要用`^`这个元字符，<strong>只有在`[`和`]`内部适用的`^`才表示非的关系</strong>。子表达式匹配的非关系就要用到前负向查找子表达式`(?!regex)`和后负向查找子表达式`(?<!regex)`。
* 或的话。比如同时匹配`a`和`b`两种情况就可以用<code>(a|b)</code>子表达式。


## 总结

学习正则要从简单到难，比如从一对一但字符，再到多对多的字符，然后通过分组，回溯引用和逻辑处理方式来学习高级的正则表达式。 
